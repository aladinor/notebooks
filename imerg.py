...
dt0 = datetime(2022, 4, 4)  # upload from this date (ignored if resume_upload==True)
dt1 = dt0 + timedelta(days=2)  # upload up to this date (excluded)
wd = '/media/alfonso/drive/Alfonso/imerg/'
gcs = False # if False, save locally instead of uploading to GCS
...
class State(object):
    def __init__(self, dt0, dt1):
        ...
        self.fields = ['precipitationCal', 'precipitationUncal', 'randomError', 'HQprecipitation', 'HQprecipSource',
                       'HQobservationTime', 'IRprecipitation', 'IRkalmanFilterWeight', 'probabilityLiquidPrecipitation',
        ...
        if not state.download_filenames:
            ...
            for fname in state.download_filenames[:state.date_nb]:
                try:
                    ...
                    last_ds = xr.Dataset({field: (['lon', 'lat'], f[f'Grid/{field}'][0]) for field in state.fields},
                                         coords={'lon': f['Grid/lon'], 'lat': f['Grid/lat']}).transpose()
                    ds.append(last_ds)
                    f.close()
                except:
                    ds.append(last_ds)
    ...
    if not os.path.exists(f'{wd}/gpm_imerg/late/chunk_time'):
        ...
        if gcs:
            subprocess.check_call(
                f'gsutil -m cp -r {wd}/gpm_imerg/late/chunk_time gs://pangeo-data/gpm_imerg/late/chunk_time'.split(),
        ...
        for field in state.fields:
            ...
            if gcs:
                ...
                while not done:
                    ...
                    try:
                        subprocess.check_call(
                            f'gsutil -m cp -r {wd}/gpm_imerg/late/chunk_time/{field} gs://pangeo-data/gpm_imerg/late/chunk_time'.split(),
                            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                    except:
                        done = False
            else:
...
async def main():
    ...
    if resume_upload:
        ...
        if gcs:
            try:
                subprocess.check_call('gsutil -m rm -rf gs://pangeo-data/gpm_imerg/late'.split(),
                                      stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            except:
                pass
            # gcloud init
    ...
    if gcs:
        ...
        while not done:
            ...
            try:
                subprocess.check_call('gsutil -m rm -r gs://pangeo-data/gpm_imerg/late/chunk_time/time'.split(),
                                      stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                subprocess.check_call(
                    f'gsutil -m cp -r {wd}/gpm_imerg/late/time/time gs://pangeo-data/gpm_imerg/late/chunk_time'.split(),
                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            except:
                done = False
    else:
